...
function __promptline_host {
  ...
  if [ ! $only_if_ssh -o -n "${SSH_CLIENT}" ]; then
    if [[ -n ${ZSH_VERSION-} ]]; then print %m; elif [[ -n ${FISH_VERSION-} ]]; 
    then hostname -s; else printf "%s" \\h; fi
  fi
}
...
function __promptline_ps1 {
  ...
  ## section "a" slices
  #__promptline_wrapper "[\D{⏰  %T}]" "$slice_prefix" "$slice_suffix" &&
...
function __promptline {
  ...
  local a_bg="${wrap}48;5${end_wrap}"
  local a_sep_fg="${wrap}38;5;31${end_wrap}"
  local t_fg="${wrap}38;5;220${end_wrap}"
  local t_bg="${wrap}48;5;31${end_wrap}"
  local t_sep_fg="${wrap}38;5;31${end_wrap}"
  ...
  local reset="${wrap}0${end_wrap}${space}${a_bg}\n \$${c_fg}"
  
  PS1='$(__print_pre_prompt)'$'\n'"$(__promptline_ps1)"
}
